      ...
      <PageHeader
        eyebrow={
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            <Pill tone="neutral">{riskLabel(invoice.riskCategory)}</Pill>
            <Pill tone="info">Buyer rating {invoice.buyerRating}</Pill>
          </div>
        }
          ...
          live ? (
            <>
              <Link to="/auction/$id" params={{ id: invoice.id }}>
                <Button variant="outline">Open auction room</Button>
              </Link>
              <Button onClick={() => setBidOpen(true)}>Place bid</Button>
            </>
          ) : null
        }
      />
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-xl border border-border bg-card p-5 shadow-card">
            ...
            <ul className="mt-3 space-y-3">
              {[
                { label: "Invoice document", hash: invoice.docHash },
                { label: "Purchase order", hash: invoice.poHash },
                { label: "Delivery confirmation", hash: invoice.deliveryHash },
              ].map((doc) => (
